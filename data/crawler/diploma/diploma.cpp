...
int main ( int argc, char *argv[] )
{
	if ( argc != 3 )
	{
		cout << "Usage: a.out YEAR DIR_NAME\nExample: a.out 15 2\nDownload 2015 batch of 2nd year students\n" ;
		exit(1) ;
	}
	...
	for ( i = 0 ; i < num_bran ; i ++ )
	{
		...
		for ( j = 1 ; j <= limit[i] ; j ++ )
		{
			sprintf(cmd, "wget -O %d/%s/4JC%02d%s4%02d --post-data 'USN=4JC%02d%s4%02d&submit_result=Fetch Result' http://sjce.ac.in/view-results", dir, br, year, br, j, year, br, j);
			system(cmd) ;
		}
	}
